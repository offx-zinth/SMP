...
class RateLimitRetryableError(RuntimeError):
    """Raised when a remote model call appears to be rate limited (HTTP 429)."""
...
class SMPMemory:
    ...
    @retry(
        retry=retry_if_exception_type(RateLimitRetryableError),
        wait=wait_exponential(multiplier=1, min=1, max=30),
        stop=stop_after_attempt(6),
        reraise=True,
    )
    def _generate_summary_with_backoff(self, prompt: str) -> str:
        try:
            ...
            if self._is_rate_limited_error(exc):
                raise RateLimitRetryableError("Gemini API rate limit encountered.") from exc
            raise
    ...
    def _generate_compressed_context_with_backoff(self, prompt: str) -> str:
        try:
            response = self._genai_client.models.generate_content(
                model="gemini-2.5-flash",
                contents=prompt,
    ...
    def get_compressed_context(self, file_path: str) -> dict[str, Any]:
        ...
        text = self._generate_compressed_context_with_backoff(prompt)
        try:
